...
class PyPdfFormCreator:
    ...
    def add_text_fields_count_metadata(self, count: int) -> None:
        """Attach a metadata entry with the number of text fields detected.

        Stored under /CommonFormsTextFieldCount as a string value so that it is
        broadly readable by PDF readers and downstream tooling.
        """
        try:
            self.writer.add_metadata({"/CommonFormsTextFieldCount": str(int(count))})
        except Exception:
            # If metadata cannot be attached for any reason, fail soft.
            pass
